...
const VoiceRecorder = () => {
  ...
  const [messages, setMessages] = useState<VoiceMessage[]>(() => {
    ...
    return saved
      ? JSON.parse(saved).map((m: VoiceMessage) => ({
          ...m,
          createdAt: new Date(m.createdAt),
        }))
      : [];
  });
  const [currentMessage, setCurrentMessage] = useState<VoiceMessage | null>(
    null,
  );
        ...
        >
          ...
          {messages.length === 0 ? (
            <p className="text-center text-muted-foreground py-12">
              No voice messages yet. Start recording to create your first
              message!
            </p>
          ) : (
