...
const FlashcardGenerator = () => {
  const [sets, setSets] = useState<FlashcardSet[]>(() => {
    ...
    return saved
      ? JSON.parse(saved).map((s: FlashcardSet) => ({
          ...s,
        ...
        sentences.forEach((sentence, i) => {
          if (i < sentences.length - 1) {
            ...
            if (question.length > 10 && answer.length > 10) {
              ...
              const choices = [answer, ...wrongAnswers].sort(
                () => Math.random() - 0.5,
              );
      ...
      if (words && words.length > 0) {
        words.slice(0, 5).forEach((word, i) => {
          const context = paragraph.substring(
            ...
            Math.min(
              paragraph.length,
              paragraph.indexOf(word) + word.length + 100,
            ),
          );
  ...
  const extractTextFromPDF = async (file: File): Promise<string> => {
    try {
      ...
      if (error.message?.includes("Invalid PDF")) {
        ...
        throw new Error(
          `Failed to extract text: ${error.message || "Unknown error"}. Please try a different PDF file or convert it to text format.`,
        );
      }
    }
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (file.type === "text/plain" || file.name.endsWith(".txt")) {
        ...
        alert(
          "Image OCR requires additional setup. Please copy and paste the text content instead.",
        );
      ...
      <div className="container max-w-5xl mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-muted-foreground font-serif-italic">
            Upload your lessons and I'll create flashcards for you, Zainab!
          </p>
        </motion.div>
              ...
              >
                <div className="text-center mb-6">
                  ...
                  <div className="text-2xl font-medium mb-6">
                    {currentSet.flashcards[currentCardIndex].front}
                  </div>
                </div>

                {currentSet.flashcards[currentCardIndex].choices &&
                currentSet.flashcards[currentCardIndex].choices!.length > 0 ? (
                  <>
                    <div className="space-y-3 mb-6">
                      {currentSet.flashcards[currentCardIndex].choices!.map(
                        (choice, idx) => {
                          // Normalize strings for comparison
                          const normalizedChoice = choice.trim().toLowerCase();
                          const normalizedAnswer = currentSet.flashcards[
                            currentCardIndex
                          ].back
                            .trim()
                            .toLowerCase();
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          if (currentSet) {
                            markMastery(
                              currentSet.flashcards[currentCardIndex].id,
                              value,
                            );
                            if (
                              currentCardIndex <
