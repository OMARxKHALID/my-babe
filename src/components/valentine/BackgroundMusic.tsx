...
class MusicGenerator {
  ...
  playMelody(vibe: "romantic" | "inspecting" = "romantic") {
    ...
    melody.forEach((noteIndex, i) => {
      const freq =
        vibe === "romantic"
          ? this.notes.romantic[noteIndex]
          : this.notes.inspecting[noteIndex];
      // Skip muted steps
      if (freq === 0) return;
  ...
  stop() {
    ...
    this.oscillators.forEach((osc) => {
      try {
        ...
        // Ignore errors when stopping oscillators that haven't started
      }
    });
    this.oscillators = [];
  }
